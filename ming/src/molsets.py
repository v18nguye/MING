...
def get_all_metrics(gen, dataset, k=None, n_jobs=1,
    ...
    if pool is None:
        if n_jobs != 1:
            ...
            pool = 1
    metrics['valid'] = fraction_valid(gen, n_jobs=pool) # always=1
    gen = remove_invalid(gen, canonize=True) # smiles
    print('number of gen to compute: ', len(gen))
    if not isinstance(k, (list, tuple)):
        k = [k]
    for _k in k:
        metrics[f'unique@{_k}'] = fraction_unique(gen, _k, pool)
    
    mols = mapper(pool)(get_mol, gen) # RDkit
